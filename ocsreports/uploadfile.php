...
{	
	...
	else if((strcasecmp($ext,"exe")==0)&&(strcasecmp($cp,"ocsagent")==0)) {
		$fname="ocsagent.exe";
		$platform="windows";
	}
...
<?
	$queryD = "SELECT '-' as version, 'windows' as os,name FROM deploy";
	...
	do
	{
		...
		if( strcasecmp($item->name,"ocsagent.exe") ==0)
	...
	?>		
		...
		<td align=center><a href=# OnClick='window.open("download.php?dl=1&v=<?=$item->version?>&o=<?=$item->os?>&n=<?=$item->name?>","fen","location=0,status=0,scrollbars=0,menubar=0,resizable=0,width=1,height=1")'><img src=image/Gest_admin1.png></a></FONT></td>
